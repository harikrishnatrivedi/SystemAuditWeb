...
public class DeviceInfoDAOImpl extends GenericDAOImpl<DeviceInfo, Integer> implements DeviceInfoDAO {
	...
	public List<DeviceInfo> listSuspiciousSytemByLatestSchedule() {
		...
		return getCurrentSession()
				.createCriteria(DeviceInfo.class,
						"DeviceInfo")
				.createAlias("lstObjFileDetailsExcludeGoodView", "FileDetails")
	...
	public DeviceInfo getDeviceInfoById(int paramIntId) {
		return (DeviceInfo) getCurrentSession().createCriteria(DeviceInfo.class).add(Restrictions.eq("compId", paramIntId)).uniqueResult();
	}

	public void removeDeviceInfo(int paramIntId) {
		DeviceInfo ed = (DeviceInfo) getCurrentSession().createCriteria(DeviceInfo.class).add(Restrictions.eq("compId", paramIntId)).uniqueResult();
		if (ed != null) {
